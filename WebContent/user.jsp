	...
	<div class="container" id="content">
		<div class="clearfix">
			<div class="col-md-12 column">
				<ul id="myTab" class="nav nav-tabs">
					...
					<li><a href="#activity" data-toggle="tab"><span
							class="glyphicon glyphicon-leaf"></span>成就</a></li>
					<c:if
						test="${queryUserInfo.uid==NowLoginUser.uid and not empty NowLoginUser}">
						<li><a href="#edit" data-toggle="tab"><span
								class="glyphicon glyphicon-cog"></span>编辑</a></li>
					</c:if>
				</ul>
			</div>
		</div>
		<div id="myTabContent" class="tab-content clearfix">
					...
					<div class="clearfix">
						<div class="col-sidebar col-lg-3">
							<div id="avatar-card" class="avatar-card">
								<div class="avatar">
									<a href="javascript:;"
										onclick="touserjump(${queryUserInfo.uid})"> <input
										type="hidden" id="queryUserId" value="${queryUserInfo.uid}">
										<img
										src="<c:if test="${not empty queryUserInfo.uhead}"><%=basePath%>files/${queryUserInfo.uhead}</c:if><c:if test="${empty queryUserInfo.uhead}"><%=basePath%>files/ITForum.jpg</c:if>"
										alt="" width="164" height="164" class="avatar-user">
									</a>
								</div>
								<div class="reputation zebra_tips1" title="该用户所获得的荣誉<br>获得方法：发帖+回复+建版块">
									${userTopic.totalRecordCount+userReplys.totalRecordCount+sessionCount} <span class="label-uppercase">reputation</span>
								</div>

								<div class="g-row _gutters ai-start fl-none -row-first">
									<c:if test="${queryUserInfo.uissectioner ==0}">
										<div class="g-col g-row g-center badge1-alternate zebra_tips1"
											title="普通用户">
											<a class="g-col g-center -total"><i
												class="	glyphicon glyphicon-magnet"></i></a>
										</div>
									</c:if>
									<c:if test="${queryUserInfo.uissectioner ==1}">
										<div class="g-col g-row g-center badge1-alternate zebra_tips1"
											title="版主">
											<a class="g-col g-center -total"><i
												class="glyphicon glyphicon-tower"></i></a>
										</div>
									</c:if>
									<div class="g-col g-row g-center badge2-alternate zebra_tips1"
										title="举报该用户">
										<a href="javaScript:;" data-toggle="modal"
											class="g-col g-center -total" id="toMyModal"><i
											class="glyphicon glyphicon-warning-sign"></i></a>
									</div>
									<div class="g-col g-row g-center badge3-alternate zebra_tips1"
										title="发送私信给他">
										<a href="javaScript:;"
											onclick="toInfoCenterjump(${queryUserInfo.uid})"
											class="g-col g-center -total"><i
								...
								<div class="user-links">
									<div class="user-stats">
										<div class="row" style="margin: 0;">
											<div class="stat">
												<span class="number"><i
													class="glyphicon glyphicon-user"></i> <i id="queryUserName">${queryUserInfo.uname}</i></span>
											</div>
									...
									<div class="user-link">
										<ul class="list-unstyled">
											<li><span class="	glyphicon glyphicon-fire"></span> 性别：<c:if
													test="${queryUserInfo.usex==1 }">男</c:if> <c:if
													test="${queryUserInfo.usex==0 }">女</c:if></li>

											<li><span class="glyphicon glyphicon-file"></span>
												发帖个数：${userTopic.totalRecordCount}</li>
											<li><span class="glyphicon glyphicon-comment"></span>
												回帖个数：${userReplys.totalRecordCount}</li>
											<li><span class="glyphicon glyphicon-info-sign"></span>
												用户状态： <c:if test="${queryUserInfo.ustate > 0}">良好</c:if> <c:if
													test="${queryUserInfo.ustate < -10}">警告</c:if></li>
											<li><span class="glyphicon glyphicon-gift"></span>
												出生日期：${queryUserInfo.ubirthady}</li>
											<li><span class="glyphicon glyphicon-time"></span> 注册时间：<i
												id="regist_time">${queryUserInfo.uregdate}</i></li>
										</ul>
									</div>
							...
							<div class="module" id="interesting-tags">
								...
								<div>
									<c:if test="${empty userFollowSession}">
										<br>
										<br>
										<p style="color: #9e9e9e;">用户暂时没有关注的版块</p>
									</c:if>
									<c:forEach items="${userFollowSession}" var="follow">
										<a href="javaScript:;" class="label label-success" rel="tag"
											onclick="tosessionjump(${follow.sid})"> ${follow.sname} </a>
									</c:forEach>
								</div>
							</div>
							<div class="module" id="myNew-tags">
								...
								<div>
									<c:if test="${empty sessionsCreatedByUser}">
										<br>
										<br>
										<p style="color: #9e9e9e;">用户暂时不是版主</p>
									</c:if>
									<c:forEach items="${sessionsCreatedByUser}" var="session">
										<a href="javaScript:;" class="label label-success" rel="tag"
											onclick="tosessionjump(${session.sid})"> ${session.sname}
										</a>
									</c:forEach>
								</div>
							</div>
						...
						<div class="col-md-9">
							...
							<div class="tab-content">
								<div class="tab-pane fade in active">
									<table class="table table-hover table-condensed table-striped">
										...
										<tbody id="tbody">
											<c:forEach items="${userTopic.data}" var="topic"
												varStatus="num">
												<tr <c:if test="${num.index%2==1}">class="success"</c:if>>
													<td><a href="javaScript:;"
														onclick="toTopicJump(${topic.tid},${topic.tsid})">${topic.ttopic}</a></td>
													<td>${topic.ttime}</td>
												</tr>
											</c:forEach>
										</tbody>
							...
							<c:if test="${not empty userTopic.data}">
								...
								<div class="col-xs-2 col-xs-offset-5"
									...
									<a class="btn btn-default" id="showMore">加载更多</a> <input
										type="hidden" value="2" id="nowPageNo">
								...
								<div class="tab-pane fade in active">
									<table class="table table-hover table-condensed table-striped">
										...
										<tbody id="reply_tbody">
											<c:forEach items="${userReplys.data}" var="reply"
												varStatus="num">
												<tr <c:if test="${num.index%2==1}">class="success"</c:if>>
													<td><a href="javaScript:;"
														onclick="toTopicJump(${reply.rtid},${reply.rsid})">${reply.rcontent}</a></td>
													<td>${reply.rtime}</td>
												</tr>
											</c:forEach>
										</tbody>
							...
							<c:if test="${not empty userReplys.data}">
								<div class="col-xs-2 col-xs-offset-5"
									style="padding-bottom: 20px;">
									<a class="btn btn-default" id="replyShowMore">加载更多</a> <input
										type="hidden" value="2" id="replyNowPageNo">
									<div class="loadEffect" id="replyLoadEffect"
										style="display: none;">
										<span></span> <span></span> <span></span> <span></span> <span></span>
										<span></span> <span></span> <span></span>
									</div>
			...
			<div class="tab-pane fade" id="activity">
				<div class="clearfix col-xs-10 col-xs-offset-1" style="margin-bottom: 170px;">
					<div id="achievePane">
						<div class="polaroid zebra_tips1" title="发布帖子超过10个" <c:if test="${userTopic.totalRecordCount>10}">style=" opacity: 1;filter: alpha(opacity = 100);"</c:if>>
							<a href="javaScritp:;"></a>
							<div class="container">
								<p>发帖新手</p>
							</div>
						...
						<div class="polaroid zebra_tips1" title="发布帖子超过100个" <c:if test="${userTopic.totalRecordCount>100}">style=" opacity: 1;filter: alpha(opacity = 100);"</c:if>>
							<a href="javaScritp:;"></a>
							<div class="container">
								<p>发帖达人</p>
							</div>
						</div>
						<div class="polaroid zebra_tips1" title="发布帖子超过1000个" <c:if test="${userTopic.totalRecordCount>1000}">style=" opacity: 1;filter: alpha(opacity = 100);"</c:if>>
							<a href="javaScritp:;"></a>
							<div class="container">
								<p>发帖狂人</p>
							</div>
						...
						<div class="polaroid zebra_tips1" title="创建版块超过1个" <c:if test="${sessionCount>1}">style=" opacity: 1;filter: alpha(opacity = 100);"</c:if>>
							<a href="javaScritp:;"></a>
							<div class="container">
								<p>版块新手</p>
							</div>
						...
						<div class="polaroid zebra_tips1" title="创建版块超过5个" <c:if test="${sessionCount>5}">style=" opacity: 1;filter: alpha(opacity = 100);"</c:if>>
							<a href="javaScritp:;"></a>
							<div class="container">
								<p>版块达人</p>
							</div>
						</div>
						<div class="polaroid zebra_tips1" title="创建版块超过10个" <c:if test="${sessionCount>10}">style=" opacity: 1;filter: alpha(opacity = 100);"</c:if>>
							<a href="javaScritp:;"></a>
							<div class="container">
								<p>版块狂人</p>
							</div>
						</div>
						<div class="polaroid zebra_tips1" title="回复帖子超过100个" <c:if test="${userReplys.totalRecordCount>100}">style=" opacity: 1;filter: alpha(opacity = 100);"</c:if>>
							<a href="javaScritp:;"></a>
							<div class="container">
								<p>回帖新手</p>
							</div>
						...
						<div class="polaroid zebra_tips1" title="回复帖子超过1000个" <c:if test="${userReplys.totalRecordCount>1000}">style=" opacity: 1;filter: alpha(opacity = 100);"</c:if>>
							<a href="javaScritp:;"></a>
							<div class="container">
								<p>回帖达人</p>
							</div>
						</div>
						<div class="polaroid zebra_tips1" title="回复帖子超过10000个" <c:if test="${userReplys.totalRecordCount>10000}">style=" opacity: 1;filter: alpha(opacity = 100);"</c:if>>
							<a href="javaScritp:;"></a>
							<div class="container">
								<p>回帖狂人</p>
							</div>
							...
							<div class="col-md-3">
								<div class="thumbnail">
									<img
										...
										alt="图片不符合规范" id="PictureBySet"
										style="min-width: 216px; min-height: 189px; width: 216px; height: 189px;">
									<div class="caption">
										<label for="setPicture" class="btn btn-success btn-block">set
											picture</label><input type="file" id="setPicture" class="hidden"
											name="PictureBySet"
											value="${sessionScope.NowLoginUser.uhead}" />
									</div>
								</div>
							</div>
							<div class="col-md-8">
								<div class="form-group">
									<label for="userName">姓名</label><input type="text"
										...
										value="${sessionScope.NowLoginUser.uname}" />
								</div>
								<div class="form-group">
									<label for="userEmail">邮箱</label><input type="email"
										class="form-control" id="userEmail" name="userEmail"
										value="${sessionScope.NowLoginUser.uemail}" />
								</div>

								<div class="form-group">
									<label>性别</label> <input type="radio" name="userSex" value="1"
										<c:if test="${sessionScope.NowLoginUser.usex ==1}">checked="checked"</c:if>>男
									<input type="radio" name="userSex" value="0"
										<c:if test="${sessionScope.NowLoginUser.usex ==0}">checked="checked"</c:if>>女

								</div>
								<div class="form-group">
									<label for="userBirthday">生日</label><input type="date"
										class="form-control" id="userBirthday" name="userBirthday"
										value="${sessionScope.NowLoginUser.ubirthady}" />
								</div>
								<div class="form-group">
									<label for="userBio">自我介绍</label>
									<textarea class='form-control' id="userBio" name="userBio"
										onkeyup='textAreaChange(this)'
										onkeydown='textAreaChange(this)' rows='5'>${sessionScope.NowLoginUser.usatement}</textarea>
									<div class='text-right'>
										<em style='color: red'>500</em>/<span>500</span>
									</div>
								</div>
								<button type="submit" class="btn btn-default" id="setUserInfo"
				...
				<div class="modal-body">
					<form action="#" method="post" id="reportForm">
						<div class="poplayer-bd">
							<p class="text-gray">
								温馨提示：填写完整的描述，举报将得到优先处理。<br>请再次确认您的举报信息，确认信息已全部上传，以便投诉可以全部处理。
							</p>
							<div class="form-row js-row">
								<label class="form-label" for=""> <span
									class="form-need">*</span> 举报问题类别：
								</label>
								<div class="f-control">
							...
							<div class="form-row js-row">
								<label class="form-label" for=""> <span
									class="form-need-hidden">*</span> 被举报的用户：
								</label>
								<div class="f-control">
									<input class="form-input js-phone" type="text"
										placeholder="填写正确的用户">
								</div>
								<div class="form-tips">
							...
							<div class="form-row js-row">
								<label class="form-label-block"> <span class="form-need">*</span>
									问题描述 <span class="text-gray"> (为了更快速解决您的问题，请尽量提供详实描述) </span>
								</label>
								<div class="f-control">
									<textarea maxlength="250" placeholder="如：在xx版块，发布了xxx信息。"
